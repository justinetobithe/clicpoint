...
export default function UserPrescipt() {
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - state.prescriptions.length) : 0;
    ...
    const { data, loading } = useFetch({
        url: `/api/prescription/parent/${state.user.id}`
    })
    ...
    return (
        <>
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800">My Prescription</h1>
            </div>
                        ...
                        <Paper sx={{ width: '100%', mb: 2 }}>
                            <TableContainer component={Paper}>
                                ...
                                <Table sx={{ minWidth: 500, borderTop: "1px solid rgba(224, 224, 224, 1)" }} aria-label="custom pagination table">
                                    ...
                                    <TableBody>
                                        {(rowsPerPage > 0
                                            ? state.prescriptions.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
                                            : state.prescriptions
                                        ).map((row, index) => (
                                            <TableRow key={row.id}>
