...
export default function ViewAppointment() {
    ...
    const { data, loading } = useFetch({
        url: `/api/appointment/parent/${state.user.id}`
    })
    ...
    const bookAppointment = () => {
        dispatch({
            ...
            payload: {
                ...
                children: <BookAppointmentModal />
            }
        })
    }

    const editAppointmentModal = (data) => {
        dispatch({
            type: "TOGGLE_MODAL",
            payload: {
                isShown: true,
    ...
    return (
        <>
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 text-gray-800">My Appoinments</h1>
                <button onClick={() => bookAppointment()} className="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">
                    <AddIcon /> Book an appointment
                </button>
            </div>
                                    ...
                                    <TableBody>
                                        {(rowsPerPage > 0
                                            ...
                                            <TableRow key={row.id}>
                                                ...
                                                <TableCell>
                                                    {dateFormat(row.schedule, "mmmm d, yyyy")}
                                                </TableCell>
                                                <TableCell>
                                                    {row.time}
                                                </TableCell>
                                                ...
                                                <TableCell>
                                                    {
                                                        row.status == 1 && (
                                                            <>
                                                                <Tooltip title="Edit">
                                                                    <IconButton aria-label="edit" color="secondary" onClick={() => editAppointmentModal(row)}>
                                                                        <EditIcon />
                                                                    </IconButton>
                                                                </Tooltip>
                                                                {
